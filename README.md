...
- **Live updates**: the dashboard refreshes itself the moment a player
  catches or encounters a Pokémon
- **Per-species detail cards**: types, height/weight, abilities (with
  hover descriptions), egg groups, and where to find it in-game
...
The served page lives in `config/cobblesync/web/` after the first start you can
...
- Backup Sprites via [PokeAPI/sprites](https://github.com/PokeAPI/sprites).
